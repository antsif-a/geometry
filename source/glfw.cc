...
export namespace glfw
{
    ...
    struct window_view {
        ...
        dvec2 get_cursor_position() {
            dvec2 pos;
            glfwGetCursorPos(handle, &pos.x, &pos.y);
            return pos;
        }

        ivec2 get_framebuffer_size() {
            ivec2 size;
    ...
    struct window: public window_view {
        window(GLFWwindow * handle) : window_view(handle) {
            logger::debug("window created (handle = {})", (void*)handle);
        }
    ...
    window create_window(
        ...
        if (!glfwInit())
            throw glfw::error("Could not initialize window framework");
